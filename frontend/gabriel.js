...
function values_changed() {
    ...
    pluginData["all_sources"] = document.getElementById("all_sources").checked;


    localStorage.setItem(pluginName, JSON.stringify(pluginData));
}

function toggle_all_sources()
{
    var pluginData = JSON.parse(localStorage.getItem(pluginName));
    var checked = document.getElementById("all_sources").checked;
    pluginData["all_sources"] = checked;
    var allSources = document.getElementsByClassName("sources");
    if (pluginData["sources"] == undefined) {
        pluginData["sources"] = {};
    }
    for (var i = 0; i < allSources.length; i++) {
        allSources[i].checked = checked;
        var id = allSources[i].id;
        pluginData["sources"][id] = checked;
    }
    localStorage.setItem(pluginName, JSON.stringify(pluginData));
}
...
function load_values() {
    ...
    if (pluginData["all_sources"] != undefined) {
        document.getElementById("all_sources").checked = pluginData["all_sources"];
    }
